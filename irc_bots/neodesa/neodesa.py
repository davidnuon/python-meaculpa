...
name = "meeting_%s.html" % time.strftime("%I_%M_%p")
print name
...
def parsemsg(msg) :
    ...
    log = open(name, "a")
    log.write('<span style="font-family:monospace;">[%s] &lt;%s&gt; %s</span><br>\r' % (time.strftime("%I:%M %p"), sender[0], msgpart))
    log.close()
    ...
    if msgpart[0] == '=' :
        if sender[0] in ignorel :
            send('Go away, %s' % sender[0])
                ...
                #Ends the bot
                elif gou[0] == 'end' and sender[0] == 'dn_desaku':
                    send(bot.end())
                    
               #Joins channels
                elif gou[0] == 'join' and sender[0] == 'dn_desaku':
                ...
                #Poll
                    ...
                    try :
                        print gou
                        if gou[1] == 'set' and sender[0] in owner :
                            print 'Testing'
                            cmd = cmd.replace('set ', '')
                            send(poll.set(cmd))
                            error()
                        elif gou[1] == 'de' :
                            if sender[0] in owner :
                                print poll.debug(sender[0]).owner
                                send('.de')
                            else :
                                send(poll.debug(sender[0]).other)
                        elif gou[1] == 'reset' and sender[0] in owner : 
                            send(poll.reset())
                            error()
                        elif gou[1] == 'yes' :
                            dsend(poll.vote('yes', sender[1], sender[0]))
                            print poll.vote('yes', sender[1], sender[0])
                            error()
                        elif gou[1] == 'no' :
                            dsend(poll.vote('no', sender[1], sender[0]))
                            print poll.vote('no', sender[1], sender[0])
                            error()
                        elif gou[1] in ['info', 'call'] :
                            send(poll.pollinfo())
                            error()
                        elif gou[1] == 'voters' :
                            send(poll.voterinfo())
                ...
                # The bots of Grahamotte! 
                elif gou[0] == 'prefix' :
                    if gou[1] == '+' :
                        add = gou[2].split('as')
                        print add
                        prefixi.append(add[0])
                        prefix[add[0]] = add[1]
                        send('%s\'s prefix has beed added as %s' % (add[0], add[1]))
                    elif gou[1] == 'list' :
                        for n in prefixi :
                            print prefixi, prefix, n
                            send(n + ' prefix is ' + prefix[n])
                else :
